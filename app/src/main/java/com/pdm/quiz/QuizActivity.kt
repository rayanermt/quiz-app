...
class QuizActivity : AppCompatActivity(),View.OnClickListener {
    ...
    private fun loadQuestions(){
        ...
        binding.apply {
            questionIndicatorTextview.text = "Pergunta ${currentQuestionIndex+1}/ ${questionModelList.size} "
    ...
    override fun onClick(view: View?) {
        ...
        if(clickedBtn.id==R.id.next_btn){
            // Clicou no botão de próximo
            if(selectedAnswer.isEmpty()){
                Toast.makeText(applicationContext,"Por favor escolha uma resposta para continuar",Toast.LENGTH_SHORT).show()
                return;
            }
            if(selectedAnswer == questionModelList[currentQuestionIndex].correct){
                score++
                Log.i("Pontuação: ",score.toString())
            }
